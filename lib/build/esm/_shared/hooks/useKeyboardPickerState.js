...
export function useKeyboardPickerState(props, options) {
  ...
  var handleKeyboardChange = useCallback(function (date) {
    var modFormat = format; //TODO: read from props

    if (/Y{2,4}/.test(format)) {
      var modYear = "".concat(utils.getYear(date) + 543);
      modFormat = format.replace(/YYYY/g, modYear);
      modFormat = modFormat.replace(/YYY/g, modYear);
      modFormat = modFormat.replace(/YY/g, modYear.toString().substring(2, 4));
    }

    _onChange(date, date === null ? null : utils.format(date, modFormat));
